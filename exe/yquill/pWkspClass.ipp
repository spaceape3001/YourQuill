...
namespace {
    ...
    void    class_reverses(HtmlWriter& h)
    {
        ...
        if(x_can_edit)
            test(decode_root(), true);
        h.title(cls_title_root() + "\" Reverses");
        if(x_can_edit)
            h << ControlRoot();
    ...
    void    class_sources(HtmlWriter& h)
    {
        ...
        if(x_can_edit)
            test(decode_root(), true);
        h.title(cls_title_root() + "\" Sources");
        if(x_can_edit)
            h << ControlRoot();
    ...
    void    class_targets(HtmlWriter& h)
    {
        ...
        if(x_can_edit)
            test(decode_root(), true);
        h.title(cls_title_root() + "\" Targets");
        if(x_can_edit)
            h << ControlRoot();
        if(x_can_edit && x_root){
            ClassData::Shared   whole = cdb::merged(x_class);
    ...
    void    class_use(HtmlWriter& h)
    {
        ...
        if(x_can_edit)
            test(decode_root(), true);
        h.title(cls_title_root() + "\" Dependencies");
        if(x_can_edit)
            h << ControlRoot();
        if(x_can_edit && x_root){
            ClassData::Shared   whole = cdb::merged(x_class);
