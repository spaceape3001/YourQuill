...
namespace yq {
    ...
    namespace {
        ...
        struct M {
            ...
            M()
            {
                ...
                templates_avail = dir::subdirectory_names_set(template_dirs);
        ...
        {
            QSqlDatabase    db      = QSqlDatabase::addDatabase("QSQLITE", QString("CacheDB %1").arg(thread::id()));
    ...
    namespace wksp {
        struct Init {   // Only a class to support "friend"
            static void rootList(const std::string_view& pfx=std::string_view())
            {
                const M&  m = impl();
