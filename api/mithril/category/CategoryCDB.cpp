...
namespace yq::mithril::cdb {
    ...
    Category::SharedFile      category_doc(Fragment f, cdb_options_t opts)
    {
        ...
        if(td->load(std::move(ch), fp) != std::error_code()){
            cdbError << "Unable to parse category file: " << fp;
            return Category::SharedFile();
        }
        td -> set_file(fp);
        return td;
    ...
    Category                 db_category(Document doc, bool* wasCreated)
    {
        ...
        if(folder(doc) != categories_folder()){
            cdbError << "Rejecting category " << key(doc) << " due to not being in the '.categories' folder";        
            return Category{};
        }
        ...
        if(i.step(false) == SQResult::Done){
            ...
            cdbError << "Unable to get the category from the database: " << k;
            return Category{};
        }
    }
    ...
    Category                     make_category(std::string_view k, const RootDir* rt, cdb_options_t opts, bool *wasCreated)
    {
        if(wasCreated)
            *wasCreated = false;
        if(k.empty()){
            cdbError << "Cannot create a BLANK category.";
            return Category();
        }
        if(!rt)
            rt  = wksp::root_first(DataRole::Config);
        if(!rt){
            cdbError << "No root_dir specified to create the category in!";
            return Category{};
        }
        std::string     tfn = category_filename(k);
        ...
        if(!t){
            cdbWarning << "Unable to create/find category: " << k;
            return t;
        }
        ...
        if(!td){
            cdbWarning << "Unable to create category file!";
            return t;
        }
    ...
    Category::SharedData   merged(Category t, cdb_options_t opts)
    {
        ...
        if(!(opts & DONT_LOCK)){
            lk   = Id(t).lock(false);
            if(!lk){
                cdbWarning << "Unable to acquire read lock on category: " << key(t);
                return Category::SharedData();
            }
        }
    ...
    Category::SharedFile         write(Category t, const RootDir* rt, cdb_options_t opts)
    {
        ...
        if(!d){
            cdbWarning << "write(Category '" << key(t) << "'): Has no document!";
            return Category::SharedFile();
        }
        if(rt && !rt->is_writable(DataRole::Config)){
            cdbWarning << "write(Category '" << key(t) << "'): RootDir " << rt->key << " cannot be written to!";
            return Category::SharedFile();
        }
