...
namespace yq {
    namespace update {
        ...
        void    tag_erase(Document doc)
        {
            tag_erase(cdb::tag(doc));
        }
        ...
        void                tag_notify(Fragment frag,Change chg)
        {
            ...
            Tag::SharedData data = tag_update(x, opts);
            if(!data){
        ...
        void                tag_notify_icons(Fragment frag,Change)
        {
            tag_icon(cdb::tag(cdb::document(frag), true));
        }

        void                tag_stage3(Document doc) 
        {
            Tag     x       = cdb::db_tag(doc);
            Tag::SharedData data = tag_update(x, cdb::DONT_LOCK|cdb::U_INFO|cdb::U_ICON);
            if(!data){
        ...
        {
            Tag     x       = cdb::db_tag(doc);
            Tag::SharedData data = tag_update(x, cdb::DONT_LOCK|cdb::U_LEAF);
            if(!data){
        ...
        Tag::SharedData     tag_update(Tag x, cdb_options_t opts)
        {
            if(!x)
